...
const Analytics = () => {
  ...
  const topProducts = [
    { name: 'iPhone 15 Pro', sales: 1247, revenue: 1246753, growth: '+15.3%' },
    { name: 'Samsung Galaxy S24', sales: 892, revenue: 802908, growth: '+8.7%' },
    { name: 'MacBook Air M2', sales: 456, revenue: 546744, growth: '+12.1%' },
    { name: 'AirPods Pro', sales: 1834, revenue: 458500, growth: '+22.4%' },
    { name: 'iPad Air', sales: 623, revenue: 373800, growth: '+6.8%' }
  ];

  const channelPerformance = [
    { channel: 'Online Store', revenue: 1245678, orders: 8967, conversion: 3.2, color: 'bg-blue-500' },
    { channel: 'Mobile App', revenue: 892345, orders: 6234, conversion: 4.1, color: 'bg-purple-500' },
    { channel: 'In-Store', revenue: 567890, orders: 2156, conversion: 8.7, color: 'bg-green-500' },
    { channel: 'Marketplace', revenue: 234567, orders: 1234, conversion: 2.8, color: 'bg-orange-500' }
  ];

  const customerSegments = [
    { segment: 'New Customers', count: 8945, percentage: 25.6, avgOrder: 156.78, color: 'bg-blue-500' },
    { segment: 'Returning Customers', count: 18234, percentage: 52.3, avgOrder: 234.56, color: 'bg-green-500' },
    { segment: 'VIP Customers', count: 5678, percentage: 16.3, avgOrder: 567.89, color: 'bg-purple-500' },
    { segment: 'At-Risk Customers', count: 2035, percentage: 5.8, avgOrder: 89.45, color: 'bg-red-500' }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Comprehensive insights into your retail performance</p>
        </div>
            ...
            <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div className={`p-3 rounded-lg ${kpi.trend === 'up' ? 'bg-green-100' : 'bg-red-100'}`}>
                  <Icon className={`w-6 h-6 ${kpi.trend === 'up' ? 'text-green-600' : 'text-red-600'}`} />
                </div>
            ...
            {topProducts.map((product, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">{product.sales.toLocaleString()} units sold</p>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {channelPerformance.map((channel, index) => (
            <div key={index} className="relative">
              ...
              <h3 className="font-medium text-gray-900 mb-2">{channel.channel}</h3>
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{channel.orders.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
            ...
            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{segment.count.toLocaleString()} customers ({segment.percentage}%)</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm font-medium text-gray-900">Avg Order: ${segment.avgOrder}</p>
              </div>
            </div>
          ))}
        </div>
